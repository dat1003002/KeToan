    ...
    public partial class KhoanVay : Form
    {
        ...
        private void DataLoan_CellFormatting(object sender, DataGridViewCellFormattingEventArgs e)
        {
            ...
            try
            {
                ...
                var earliestUnconfirmedPayment = context.Payments
                    ...
                    .FirstOrDefault();
                if (earliestUnconfirmedPayment != null && earliestUnconfirmedPayment.PaymentDate.HasValue && earliestUnconfirmedPayment.PaymentDate.Value.Date <= DateTime.Today.AddDays(3))
                {
        ...
        private void dataLoan_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            try
            {
                ...
                inputAmount.Text = loan.Amount.ToString("N2");
                dateStartDate.Value = loan.StartDate != default(DateTime) ? loan.StartDate : DateTime.Now;
                inputDuration.Text = loan.Duration.ToString();
                dateStop.Value = loan.EndDate != default(DateTime) ? loan.EndDate : DateTime.Now;
            }
            catch (Exception ex)
            {
                ...
                var loans = context.Loans
                    ...
                    .Select(l => new
                    {
                        ...
                        EarliestUnconfirmedPaymentDate = context.Payments
                            .Where(p => p.LoanId == l.LoanId && !p.IsConfirmed)
                            .OrderBy(p => p.EndDate)
                            .Select(p => p.EndDate)
                            .FirstOrDefault()
                    })
                    .AsEnumerable()
                    .OrderBy(l => l.EarliestUnconfirmedPaymentDate != default(DateTime) && l.EarliestUnconfirmedPaymentDate <= DateTime.Today.AddDays(3) ? 0 : 1)
                    .ThenBy(l => l.EarliestUnconfirmedPaymentDate == default(DateTime) ? DateTime.MaxValue : l.EarliestUnconfirmedPaymentDate)
                    .ThenBy(l => l.LoanName)
                    .Select(l => new
                    {
                        l.LoanId,
        ...
        private void inputDuration_TextChanged(object sender, EventArgs e)
        {
            if (int.TryParse(inputDuration.Text, out int duration) && duration > 0)
            {
                ...
                {
                    dateStop.Value = startDate.AddMonths(duration);
                }
                else if (selectedPaymentPeriod == "Quý")
                {
                    dateStop.Value = startDate.AddMonths(duration * 3);
                }
            }
        }
        ...
        private void btnEdit_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                if (decimal.TryParse(inputAmount.Text.Replace(",", ""), out decimal amount))
                    loan.Amount = amount;
                else
                {
        ...
        private void inputAmount_TextChanged(object sender, EventArgs e)
        {
            ...
            inputAmount.TextChanged -= inputAmount_TextChanged;
            inputAmount.Text = string.Format("{0:N2}", number);
            inputAmount.SelectionStart = inputAmount.Text.Length;
            inputAmount.TextChanged += inputAmount_TextChanged;
        }
